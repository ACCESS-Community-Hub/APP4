...
if os.environ.get('PUBLISH').lower() in ['true', 'yes']:
    publish=True
else:
    publish=False
if publish:
    ...
    summaryncdir = f"{out_dir}/qc_files"
...
try:
    ...
    try:
        ...
        if os.path.exists(fsuccess): 
            ...
            print("W: no success list - will assume all variables failed")
    except:
        raise Exception(f"E: failed to read file {fsuccess}")
    try:
...
def compliance_check(success):
    try:
        ...
        if not os.path.exists(cmorfile):
            return [cmorfile, 2 , "file does not exist in APP_output"]
        ...
        if cmorerr == 0: 
            if publish:
                pathlib2.Path.mkdir(pathlib2.Path(cmorfiledir.replace(outpath_root, pub_dir)),
...
def create_timeseries(cmordir):
    try:
        ...
        if publish: 
            outbase = f"{qc_dir}/{outpath_structure}/{filebase}"
            pathlib2.Path.mkdir(pathlib2.Path(f"{summaryncdir}/{outpath_structure}"),
                                parents=True, exist_ok=True)
        else:
            outbase = f"{summaryncdir}/{filebase}"
        #
        # plotting first timestep
        if scalar == 0:
            plotout = f"{outbase}_{starttime}.png"
        else:
            plotout = f"{outbase}.png"
        ...
        if len(dims) == 4:
            ...
            if plotvar.dimensions[0] != 'time':
                ...
                plt.imshow(plotvar[0,:,:], origin='lower')
            plt.colorbar()
        elif len(dims) == 2:
        ...
        if scalar == 0:
            plt.suptitle(f"{var}, time {starttime} ({units})")
        else:
            plt.suptitle(f"{var} ({units})")
        plt.savefig(plotout)
        if publish:
            saveonlineplot(online_plot_dir, exptoprocess, table,
                           os.path.basename(plotout))
        plt.close()
        plotfileopen.close()
        #
        # make summary netcdf file of timeseries
        if scalar == 0:
            ncout = f"{outbase}_{starttime}-{endtime}.qc.nc"
        else:
            ncout = f"{outbase}.qc.nc"
        ...
        if 'time' in dsinfirst.variables.items()[0]:
            for v_name, varin in dsinmulti.variables.items():
                if v_name == 'time':
                    ...
                    meanout = dsout.createVariable(f"{v_name}_mean", varin.dtype, varin.dimensions[0])
                    minout = dsout.createVariable(f"{vname}_min", varin.dtype, varin.dimensions[0])
                    maxout = dsout.createVariable(f"{v_name}_max", varin.dtype, varin.dimensions[0])
                    stdevout = dsout.createVariable(f"{v_name}_stdev", varin.dtype, varin.dimensions[0])
                    masknumout = dsout.createVariable(f"{v_name}_masknum", varin.dtype, varin.dimensions[0])
        else:
            for v_name, varin in dsinmulti.variables.items():
                if v_name == var:
                    maskout = dsout.createVariable(v_name, varin.dtype, 'value')
                    meanout = dsout.createVariable(f"{v_name}_mean", varin.dtype, 'value')
                    minout = dsout.createVariable(f"{v_name}_min", varin.dtype, 'value')
                    maxout = dsout.createVariable(f"{v_name}_max", varin.dtype, 'value')
                    stdevout = dsout.createVariable(f"{v_name}_stdev", varin.dtype, 'value')
                    masknumout = dsout.createVariable(f"{v_name}_masknum", varin.dtype, 'value')
        ...
        ncmeanout = f"{outbase}_{starttime}-{endtime}_meanminmax.png"
        ncstdevout = f"{outbase}_{starttime}-{endtime}_stdev.png"
        ncmasknumout = f"{outbase}_{starttime}-{endtime}_masknum.png"
        ...
        plt.ylabel(units)
        plt.suptitle(f"Timeseries of {var} mean/min/max, time {starttime}-{endtime}")
        plt.savefig(ncmeanout)
        if publish:
            saveonlineplot(online_plot_dir, exptoprocess, table, os.path.basename(ncmeanout))
        plt.close()
        # stdev
        plt.figure()
        plt.plot(vartimerel, varstdev)
        plt.ylabel(units)
        plt.suptitle(f"Timeseries of {var} stdev, time {starttime}-{endtime}")
        plt.savefig(ncstdevout)
        if publish:
            saveonlineplot(online_plot_dir, exptoprocess, table, os.path.basename(ncstdevout))
        plt.close()
        # masknum
        plt.figure()
        plt.plot(vartimerel, varmasknum)
        plt.ylabel(units)
        plt.suptitle(f"Timeseries of {var} masknum, time {starttime}-{endtime}")
        plt.savefig(ncmasknumout)
        if publish:
            saveonlineplot(online_plot_dir, exptoprocess, table, os.path.basename(ncmasknumout))
        plt.close()    
        try:
            plotfileopen.close()
        except:
...
def check_compliance_results(results_comp):
    ...
    for r in results_comp: 
        if r[1] == 0:
            compliant.append(r[0])
        elif r[1] == 1:
            noncompliant.append([r[0],r[2]])
        elif r[1] == 2:
    ...
    for comp in compliant:
        try:
            ...
            print("file not found: ", os.path.basename(comp))
    print("\nNon-compliant files:")
...
def check_timeseries_results(results_timeser):
    ...
    for r in results_timeser: 
        if r[1] == 0:
            succ_timeser.append(r[0])
        elif r[1] == 1:
            fail_timeser.append([r[0],r[2]])
        else:
            pass
...
def main():
    ...
    print(f"local experiment being checked: {exptoprocess}")
    print(f"ncpus = {ncpus}")
    if args.compliance:
